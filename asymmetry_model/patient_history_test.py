...
import embed_explore as mbd
# args = parser.parse_args()

output_path = 'patient_reports_test_output'

op = pathlib.Path(output_path)
if not op.is_dir():
    os.mkdir(op)

logging.basicConfig(format='[%(levelname)s] %(asctime)s - %(message)s', filename=f'{output_path}/generate.log')
...
df_mag = mbd.load_mag( file_path='data/embed/tables/EMBED_OpenData_clinical.csv', scope='full', cohorts=[1,2])

logger.info('loading df_met')
df_met = mbd.load_met(file_path='data/embed/tables/EMBED_OpenData_metadata.csv', cohorts=[1,2])
