...
if __name__ == "__main__":
    ...
    mag_df.embed.summarize("Magview - after dropping rows with no description")


    mag_contra_df = embed_toolkit.correct_contralaterals(mag_df)
    mag_contra_df = ensure_key_column_dtypes(mag_contra_df)
    mag_contra_df.embed.summarize("Magview - after contralateral correction")
    ...
    # remove all disgnostic exams from mag_contra_df

    mag_contra_df = mag_contra_df[mag_contra_df['screen_exam'] == True]
    mag_contra_df.embed.summarize("Magview - after filtering to screening exams")
